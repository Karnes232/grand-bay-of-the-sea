...
import { getPrivacyPolicy } from "@/sanity/queries/Privacy-Policy/PrivacyPolicy"
...
export async function generateMetadata({
  ...
  if (!pageSeo) {
    ...
    throw new Error(
      "[metadata] SEO data came back empty for /privacy-policy. " +
...
export default async function Page({
  ...
  const { locale } = await params
  setRequestLocale(locale)
  const [structuredData, privacyPolicy, tNav] = await Promise.all([
    getStructuredData("Privacy Policy"),
    getPrivacyPolicy(),
    getTranslations("Navbar"),
  ])

  const heroImg = privacyPolicy.heroImage
  const heroSrc =
    ...
    <main id="main">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: breadcrumbJsonLd(
            [
              { name: tBc("home"), path: "" },
              { name: tBc("privacyPolicy"), path: "/privacy-policy" },
            ],
            locale,
          ),
        }}
      ...
      {heroSrc && (
        <CoursesHero
          ...
          alt={heroImg?.alt || "Privacy policy"}
          title={policyTitle}
          trustLine={privacyPolicy.eyebrow?.[locale]}
        />
      )}
      ...
      <section className="mx-auto max-w-[820px] px-6 pb-24 pt-[88px]">
        ...
        <div className="mt-14 border-t border-line pt-10">
          <p className="mb-5 text-[16.5px] text-muted">
            {privacyPolicy.contactPrompt?.[locale]}
          </p>
