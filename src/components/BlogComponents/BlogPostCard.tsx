...
const BlogPostCard = ({ blog, locale }: { blog: any; locale: string }) => {
  ...
  return (
    <Link
      ...
      className="group grid grid-cols-1 overflow-hidden rounded-[20px] border border-line bg-card no-underline transition-shadow duration-300 hover:shadow-[0_22px_48px_rgba(11,33,41,0.13)] md:grid-cols-[300px_1fr]"
    >
      <div className="relative aspect-[16/9] overflow-hidden bg-line md:aspect-auto">
        {src && (
          <Image
            src={src}
      ...
      <div className="flex flex-col justify-center px-[30px] py-[28px]">
        <h2 className="mb-[9px] line-clamp-2 font-display text-[clamp(1.25rem,2vw,1.55rem)] font-bold leading-[1.12] tracking-[-0.02em] text-fg">
          {blog.title[locale]}
        </h2>
        <p className="mb-4 line-clamp-2 text-[15px] leading-relaxed text-muted">
          {blog.description[locale]}
        </p>
        <span className="inline-flex items-center gap-2 text-[14.5px] font-semibold text-fg">
