...
const BlogCategory = ({
  ...
  return (
    <Link
      ...
      aria-label={name}
      className="group flex h-full flex-col overflow-hidden rounded-[20px] border border-line bg-card no-underline transition-all duration-300 ease-smooth hover:-translate-y-[5px] hover:shadow-[0_22px_48px_rgba(11,33,41,0.13)]"
    >
      ...
      <div className="flex flex-1 flex-col p-[26px]">
        <h3
          translate="no"
          className="mb-2 font-display text-[1.3rem] font-bold leading-[1.15] tracking-[-0.02em] text-fg"
        >
          {name}
        </h3>
        {desc && (
          <p className="line-clamp-3 flex-1 text-[14.5px] leading-relaxed text-muted">
            {desc}
          </p>
        )}
        <span className="mt-[18px] inline-flex items-center gap-2 text-[14.5px] font-semibold text-fg">
